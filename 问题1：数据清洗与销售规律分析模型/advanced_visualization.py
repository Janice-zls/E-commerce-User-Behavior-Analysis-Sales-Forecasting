...
try:
    ...
    monthly_sales = purchase_df.groupby(purchase_df['时间'].dt.month).agg(
        购买次数=('用户ID', 'count'),
        销售金额=('单价', 'sum')
    ).reset_index()

    x = monthly_sales['时间']
    ax1.fill_between(x, monthly_sales['购买次数'], alpha=0.2, color=COLORS['primary'])
    ax1.plot(x, monthly_sales['购买次数'], 'o-', linewidth=2.5, markersize=8, 
             color=COLORS['primary'], label='购买次数', zorder=5)
    ax1.set_ylabel('购买次数', fontsize=12, fontweight='bold', color=COLORS['primary'])
    ...
    ax1_twin = ax1.twinx()
    ax1_twin.plot(x, monthly_sales['销售金额']/10000, 's--', linewidth=2, markersize=7, 
                  color=COLORS['accent'], label='销售金额(万元)', zorder=5)
    ax1_twin.set_ylabel('销售金额 (万元)', fontsize=12, fontweight='bold', color=COLORS['accent'])
    ...
    ax1.set_title('月度销售趋势分析', fontsize=14, fontweight='bold', pad=12)

    # 1b: 星期销售模式（雷达图）
    ax2 = fig.add_subplot(gs[1, 0], projection='polar')
    weekly_sales = purchase_df.groupby(purchase_df['时间'].dt.dayofweek).size()
    theta = np.linspace(0, 2*np.pi, 7, endpoint=False)
    theta = np.concatenate([theta, [theta[0]]])
    values = weekly_sales.reindex(range(7), fill_value=0).values
    values = np.concatenate([values, [values[0]]])

    ax2.plot(theta, values, 'o-', linewidth=3, markersize=10, color=COLORS['secondary'], zorder=5)
    ax2.fill(theta, values, alpha=0.2, color=COLORS['secondary'])
    ax2.set_xticks(np.linspace(0, 2*np.pi, 7, endpoint=False))
    ax2.set_xticklabels(['周一', '周二', '周三', '周四', '周五', '周六', '周日'], fontsize=11)
    ax2.set_ylabel('购买次数', fontsize=11, fontweight='bold', labelpad=15)
    ax2.grid(True, alpha=0.3)
    ax2.set_theta_zero_location('N')
    ax2.set_theta_direction(-1)
    ax2.set_title('星期销售模式', fontsize=13, fontweight='bold', pad=15)

    # 1c: 季度销售构成
    ax3 = fig.add_subplot(gs[1, 1])
    quarterly_category = purchase_df.groupby([
        purchase_df['时间'].dt.year, 
        purchase_df['时间'].dt.quarter, 
        '商品类型'
    ]).size().unstack(fill_value=0)

    quarterly_category.plot(kind='bar', stacked=True, ax=ax3, 
                            color=COLORS['palette'][:len(quarterly_category.columns)], 
                            alpha=0.85, edgecolor='white', linewidth=0.5)
    ax3.set_xlabel('年份-季度', fontsize=12, fontweight='bold')
    ...
    ax3.set_xticklabels([f'{y}Q{q}' for y, q in quarterly_category.index], rotation=45, ha='right')
    ax3.set_title('季度销售构成', fontsize=13, fontweight='bold', pad=12)

    # 1d: 年度对比箱线图
    ax4 = fig.add_subplot(gs[2, :])
    year_data = []
    year_labels = []
    for year in sorted(purchase_df['时间'].dt.year.unique()):
        year_purchases = purchase_df[purchase_df['时间'].dt.year == year]['单价']
        year_data.append(year_purchases.values)
        year_labels.append(f'{year}年')

    bp = ax4.boxplot(year_data, labels=year_labels, patch_artist=True,
                     ...
                     flierprops=dict(marker='o', color=COLORS['accent'], alpha=0.5, markersize=6))

    ax4.set_xlabel('年份', fontsize=12, fontweight='bold')
    ax4.set_ylabel('客单价分布 (元)', fontsize=12, fontweight='bold')
    ax4.grid(True, alpha=0.15, axis='y', linestyle='--')
    ax4.set_title('年度客单价分布对比', fontsize=13, fontweight='bold', pad=12)

    fig.suptitle('商品销售时间维度多维分析', fontsize=16, fontweight='bold', y=0.98)
    fig.text(0.02, 0.01, '数据来源：电商平台用户行为数据 | 分析时间：2023-2024', 
             fontsize=9, style='italic', alpha=0.6)
    save_fig(fig, 'fig11_multidimensional_time_analysis.png')
except Exception as e:
    print(f"图11生成失败: {e}")
...
try:
    ...
    # 2a: 各类型客单价小提琴图
    ax = axes[0, 0]
    category_price = purchase_df.groupby('商品类型')['单价']
    categories = sorted(category_price.groups.keys())
    data = [category_price.get_group(cat).values for cat in categories]

    parts = ax.violinplot(data, positions=range(len(categories)), widths=0.7,
    ...
    ax.set_title('各商品类型客单价分布', fontsize=13, fontweight='bold', pad=10)

    # 2b: 销售频次热力图
    ax = axes[0, 1]
    heatmap_data = purchase_df.groupby([
        purchase_df['时间'].dt.month, 
        '商品类型'
    ]).size().unstack(fill_value=0)
    heatmap_data = heatmap_data.reindex(columns=sorted(heatmap_data.columns))

    sns.heatmap(heatmap_data.T, ax=ax, cmap='YlOrRd', annot=True, fmt='d',
    ...
    ax.set_title('月度-商品类型销售热力图', fontsize=13, fontweight='bold', pad=10)

    # 2c: 商品类型销售占比
    ax = axes[1, 0]
    category_counts = purchase_df['商品类型'].value_counts()
    ...
    labels = list(top_categories.index) + ['其他']
    colors_pie = COLORS['palette'][:len(labels)]
    ...
    ax.set_title('商品类型销售占比', fontsize=13, fontweight='bold', pad=10)

    # 2d: 利润率分析
    ax = axes[1, 1]
    products_df = pd.read_csv(os.path.join(DATA_DIR, 'products.csv'), encoding='utf-8-sig')
    products_df.columns = ['商品ID', '商品类型', '具体商品', '成本', '单价', '库存']
    products_df['成本'] = products_df.groupby('商品类型')['成本'].transform(lambda x: x.fillna(x.median()))
    products_df['利润率'] = (products_df['单价'] - products_df['成本']) / products_df['成本'] * 100

    category_profit = products_df.groupby('商品类型').agg(
        平均利润率=('利润率', 'mean'),
        商品数量=('商品ID', 'count'),
        平均单价=('单价', 'mean')
    ).reset_index()

    scatter = ax.scatter(category_profit['平均利润率'], category_profit['平均单价'],
                         s=category_profit['商品数量']*100, alpha=0.7,
                         c=category_profit['平均利润率'], cmap='RdBu_r',
                         edgecolors='white', linewidth=2, zorder=5)

    for i, txt in enumerate(category_profit['商品类型']):
        ax.annotate(txt, (category_profit['平均利润率'].iloc[i], category_profit['平均单价'].iloc[i]),
                    xytext=(5, 5), textcoords='offset points', fontsize=9, fontweight='bold')

    ax.set_xlabel('平均利润率 (%)', fontsize=12, fontweight='bold')
    ax.set_ylabel('平均单价 (元)', fontsize=12, fontweight='bold')
    ax.grid(True, alpha=0.2, linestyle='--')
    cbar = plt.colorbar(scatter, ax=ax)
    cbar.set_label('利润率 (%)', fontsize=10, fontweight='bold')
    ax.set_title('商品类型利润率分析', fontsize=13, fontweight='bold', pad=10)

    fig.suptitle('商品类型深度分析', fontsize=16, fontweight='bold', y=0.98)
    fig.text(0.02, 0.01, '数据来源：电商平台用户行为数据', fontsize=9, style='italic', alpha=0.6)
    save_fig(fig, 'fig12_category_deep_analysis.png')
except Exception as e:
    print(f"图12生成失败: {e}")
...
try:
    ...
    x_data = purchase_copy['年龄']
    y_data = purchase_copy['单价']
    ...
    for gender in ['男', '女']:
        for membership in ['普通', '会员']:
            subset = purchase_copy[(purchase_copy['性别'] == gender) & (purchase_copy['权益'] == membership)]['单价']
            if len(subset) > 0:
    ...
    payment_data = purchase_copy[purchase_copy['备注'].notna() & (purchase_copy['备注'] != '')]
    ...
    member_data = purchase_copy[purchase_copy['权益'] == '会员']['单价']
    regular_data = purchase_copy[purchase_copy['权益'] == '普通']['单价']
    ...
    sns.histplot(data=purchase_copy, x='年龄', kde=True, ax=ax6,
